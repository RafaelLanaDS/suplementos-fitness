...
Projeto: Landing page de suplementos fitness (Apex Nutrition)

Objetivo:
Criar uma landing page moderna e responsiva focada em conversão de produtos fitness (suplementos), com visual premium dark.

Público-alvo:
Homens e mulheres que treinam, buscam hipertrofia, performance e estética corporal.

Estilo visual:
- Dark mode predominante
- Contraste com vermelho (ação/energia)
- Visual forte, atlético e minimalista

- Header (logo + menu + ícones)
- Hero (imagem forte + headline + CTA)
- Benefícios rápidos (ícones)
- Destaques / Produtos (grid)
- Seções promocionais (banners)
- Newsletter / captura de leads
- Footer completo

🧩 COMPONENTES OBRIGATÓRIOS
🔥 Hero Section
imagem de atleta ao fundo
overlay escuro
título forte (ex: “ULTRAPASSE SEUS LIMITES”)
...
Whey Protein destaque
Creatina
Treinos

👉 estilo “banner dividido”

📩 Newsletter
input de email
botão CTA vermelho
fundo escuro
🎬 ANIMAÇÕES (IMPORTANTE)
fade-in nos cards ao scroll
hover nos produtos (scale + shadow)
botões com transição suave
transform: translateY(0);
transition: transform 0.3s ease;
⚙️ JAVASCRIPT (COMPORTAMENTOS)

Implementar:

scroll animations (IntersectionObserver)
hover states
possível filtro de produtos
botão CTA interativo
♿ ACESSIBILIDADE
contraste alto (texto branco + fundo escuro)
...
NÃO usar frameworks
NÃO usar jQuery
NÃO usar valores hardcoded (usar variables)
NÃO usar div desnecessária
